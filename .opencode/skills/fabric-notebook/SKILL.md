...
> For general `fab` CLI deployment and workspace management commands, see the **fabric-cli** skill.
...
The Fabric Livy API provides interactive Spark sessions but has significant restrictions:
...
# Note: some archives use no underscores (currencyexchange.parquet, not currency_exchange.parquet)
TABLES = {
...
for table, fname in TABLES.items():
    urllib.request.urlretrieve(BASE_URL + fname, f"{EXTRACT_PATH}{fname}")
...
for target_table, filename in TABLES.items():
    ...
    pdf = pd.read_parquet(parquet_path)       # pandas reads locally — no auth needed
...
fab job run "MyWorkspace.Workspace/01_ingest_contoso.Notebook"

# Check status of a previous run
fab job status "MyWorkspace.Workspace/01_ingest_contoso.Notebook" --run-id <run-id>
```
...
Query via the Warehouse SQL endpoint (cross-database reference):
```sql
SELECT COUNT(*) FROM contoso_lakehouse.dbo.bronze_sales
```

Or check in a Livy session (read-only, no writes):
```python
# Quick check via spark.sql — tables must already exist
spark.table("bronze_sales").count()
```
...
**Important:** Archive extracts without underscores — `currencyexchange.parquet` not `currency_exchange.parquet`.

### Bronze table column naming
All Contoso V2 bronze columns are **CamelCase**: `OrderKey`, `CustomerKey`, `ProductKey`, `StoreKey`,
...
When referencing bronze columns in dbt tests or raw SQL, always use the original CamelCase names.
Silver models are where you rename to `snake_case`.
...
**Symptom:** Auth error or `AnalysisException` on write.
